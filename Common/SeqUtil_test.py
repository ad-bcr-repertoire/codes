...
from Common.ProcessUtil import run_igblast, run_igblastp
...
class Helper(object):
    ...
    def annotate_regions(self, chain_type: ChainType, sequence_list: List[str], annotation_list: List[str], **kwargs) -> TableData:
        ...
        if chain_type in [ChainType.HUMAN_HEAVY, ChainType.HUMAN_LIGHT,
                          ChainType.HUMAN_BETA, ChainType.HUMAN_ALPHA,
    ...
    def annotate_regions_protein(self, chain_type: ChainType, sequence_list: List[str], annotation_list: List[str], **kwargs) -> TableData:
        """
        :param chain_type:
        :param sequence_list:
        :return:
        ...
        if chain_type in [ChainType.HUMAN_HEAVY, ChainType.HUMAN_LIGHT,
            ...
            tp_query.close()
            run_igblastp(chain_type, tp_query.name, file_blast, domain_system, **blast_kwargs)
            print('[LOG] make %s done' % file_blast)

            result_list = self.parse_igblastp(chain_type=chain_type, igblast_output=file_blast, domain_system=domain_system)
            ...
            for sequence, blast_result in zip(sequence_list, result_list):
                ...
                for r in ['FR1', 'FR2', 'FR3', 'CDR1', 'CDR2']:
                    if ('%s_from' % r) in blast_result and ('%s_to' % r) in blast_result:
                        blast_result[r + '_AA'] = sequence[blast_result['%s_from' % r]:blast_result['%s_to' % r]]

                try:
                    blast_result['CDR3_AA_temp'] = sequence[blast_result['FR3_to']+2:blast_result['FR3_to']+30]
                except KeyError:
                    blast_result['CDR3_AA_temp'] = 'N/A'
    ...
    def annotate_regions_tcr(self, chain_type: ChainType, sequence_list: List[str], annotation_list: List[str], **kwargs) -> TableData:
        """
        :param chain_type:
        :param sequence_list:
        :return:
        ...
        if chain_type in [ChainType.HUMAN_BETA, ChainType.HUMAN_ALPHA, ChainType.MOUSE_BETA, ChainType.MOUSE_ALPHA, ChainType.HUMAN_GAMMA, ChainType.HUMAN_DELTA]:
            tp_query = tempfile.NamedTemporaryFile('wt', suffix='.fasta', delete=False)
            if 'file_blast' in kwargs:
                file_blast = kwargs['file_blast']
            else:
            ...
            result_list = self.parse_igblast_tcr(chain_type=chain_type,
                                                 igblast_output=file_blast,
                                                 domain_system=domain_system)
            ...
            for sequence, blast_result in zip(sequence_list, result_list):
                ...
                if len(sequence) % 3 == 0:
                    ...
                    frame = 'Out'
                blast_result['query'] = sequence
                blast_result['full_AA'] = self.translate(sequence[1:])
                blast_result['frame'] = frame
                blast_result['stop_codon'] = '*' in aa_seq
                if 'FR4_from' in blast_result:
                    blast_result['FR4'] = sequence[blast_result['FR4_from']:blast_result['FR4_from'] + 33]
    ...
    def annotate_regions_for_short_read(self, chain_type: ChainType, sequence_list: List[str], annotation_list: List[str], **kwargs) -> TableData:
        """
        :param chain_type:
        :param sequence_list:
        :return:
        ...
        if chain_type in [ChainType.HUMAN_HEAVY, ChainType.HUMAN_LIGHT, ChainType.CHICKEN_HEAVY,
                          ChainType.CHICKEN_LIGHT, ChainType.RABBIT_HEAVY, ChainType.RABBIT_KAPPA]:
            tp_query = tempfile.NamedTemporaryFile('wt', suffix='.fasta', delete=False)
            if 'file_blast' in kwargs:
                file_blast = kwargs['file_blast']
            else:
    ...
    def annotate_regions_chicken(self, seq_list, v_type, result_dir='/home/IP-team/data/blast_db/blast_temp/'):
        ...
        for region in region_list:
            ...
            cline = NcbiblastnCommandline(num_threads=THREADS, query=os.path.join(query_dir, 'query.fasta'), db=db_file,
                                          evalue=0.1 ** E_VAL, \
                                          out=out_file, gapopen=1, gapextend=2, word_size=5, num_descriptions=NUM_ALIGN,
    ...
    def blast_run(self, query_list: list(), db_list: list(), m_type: str, **kwargs):
        if 'file_blast' in kwargs:
            file_blast = kwargs['file_blast']
        else:
            tp = tempfile.NamedTemporaryFile('wt')
        ...
        if 'blast_dir' in kwargs:
            blast_dir = kwargs['blast_dir']
        else:
            tpd = tempfile.TemporaryDirectory('wt')
            blast_dir = tpd.name
            tpd.cleanup()
    ...
    def parse_blast_result(self, result_file: str, m_type: str):
        handle = open(result_file, 'r')
        blast_parser = NCBIStandalone.BlastParser()
        iterator = NCBIStandalone.Iterator(handle, blast_parser)
        ...
        for i, each in enumerate(iterator):
            ...
            if len(each.alignments) == 0:
                ...
                for alignment in each.alignments:
                    sbjctTitle = alignment.title[1:].rstrip("\n")
                    hsps = alignment.hsps[0]
                    numAlignments += 1
                    queryStart = hsps.query_start
                    queryEnd = hsps.query_end
                    ...
                    for i, eMatch in enumerate(hsps.match):
                        if eMatch == ' ':
                            ...
                            queryMismatch = hsps.query[i]
                            sbjctMismatch = hsps.sbjct[i]
                            mismatchList.append({'query': queryMismatch, 'sbjct': sbjctMismatch, 'query_loc': queryLoc,
                                                 'sbjct_loc': sbjctLoc})
                    alignmentList.append({'matched': matched, 'whole': whole, 'sbjct_title': sbjctTitle,
                                          'query_start': queryStart, 'query_end': queryEnd,
                                          'sbjct_start': sbjctStart, 'sbjct_end': sbjctEnd,
                                          'num_mismatches': numMismatches, 'mismatches': mismatchList,
                                          'match_pattern': hsps.match})
                tempSet = {'num_alignments': numAlignments, 'alignments': alignmentList}
                resultList.append(tempSet)
        return resultList
    ...
    def annotate_regions_chicken_loose(self, seq_list, v_type, result_dir='/home/IP-team/data/blast_db/blast_temp/'):
        ...
        for i in range(len(seq_list)):
            temp = {'fr1_s': None, 'fr1_e': None, 'fr2_s': None, 'fr2_e': None, 'fr3_s': None, 'fr3_e': None,
                    'fr4_s': None, 'fr4_e': None}
            ext_index.append(temp)
        ...
        for region in region_list:
            ...
            cline = NcbiblastnCommandline(num_threads=THREADS, query=os.path.join(query_dir, 'query.fasta'), db=db_file,
                                          evalue=0.1 ** E_VAL, \
                                          out=out_file, gapopen=1, gapextend=2, word_size=10,
                                          num_descriptions=NUM_ALIGN, num_alignments=NUM_ALIGN)
    ...
    def parse_igblast(self, chain_type: ChainType, igblast_output: str, **kwargs):
        ...
        with open(igblast_output, 'r') as igb_handle:
            if chain_type in [ChainType.HUMAN_LIGHT, ChainType.HUMAN_HEAVY, ChainType.RABBIT_HEAVY, ChainType.RABBIT_KAPPA, ChainType.MOUSE_C57BL6_HEAVY]:
                while True:
                    ...
                    if line[:len('Query= ')] == 'Query= ':
                        ...
                        result = {'query': line[len('Query= '):-1], 'hit': False, 'V_gene': '', 'J_gene': '',
                                  'CDR3_NT': '-'}
                        if chain_type == ChainType.HUMAN_HEAVY or chain_type == ChainType.RABBIT_HEAVY or ChainType.MOUSE_C57BL6_HEAVY:
                            result['D_gene'] = ''
                    elif 'No hits found' in line:
                        result['hit'] = False
                        # addition point
                        result['disatnce_from_V_gene'] = 'N/A'
                        ...
                        if next_line_sp[0] == 'CDR3':
                            result['hit'] = True
                            result['CDR3_NT'] = next_line_sp[1]
                            result['CDR3_AA'] = self.translate(result['CDR3_NT'])
                        ...
                        result['aligned_length_to_V_gene'] = totalAlignedLength
                        result['distance_from_V_gene'] = totalDistanceFromVGene

                    elif line[:len('V-(D)-J rearrangement summary')] == 'V-(D)-J rearrangement summary':
                        next_line_sp = igb_handle.readline().split('\t')
                        if chain_type == ChainType.HUMAN_HEAVY or chain_type == ChainType.RABBIT_HEAVY or chain_type == ChainType.MOUSE_C57BL6_HEAVY:
                            result['V_gene'] = next_line_sp[0]
                            result['D_gene'] = next_line_sp[1]
                            result['J_gene'] = next_line_sp[2]
                            result['VJ_frame'] = next_line_sp[5].split('-')[0]
                        elif chain_type == ChainType.HUMAN_LIGHT or ChainType.RABBIT_KAPPA:
                            result['V_gene'] = next_line_sp[0]
                            result['J_gene'] = next_line_sp[1]
    ...
    def parse_igblastp(self, chain_type: ChainType, igblast_output: str, **kwargs):
        result_list = []
        result = None
            ...
            if chain_type in [ChainType.HUMAN_LIGHT, ChainType.HUMAN_HEAVY]:
                while True:
                    ...
                    if line[:len('Query= ')] == 'Query= ':
                        if result is not None:
                            result_list.append(result)
                        result = {'query': line[len('Query= '):-1], 'hit': False, 'V_gene': 'N/A', 'CDR3_AA': 'N/A'}
                    ...
                    elif line[:len('Sequences producing significant alignments:')] == 'Sequences producing significant alignments:':
                        igb_handle.readline()
                        next_line_sp = igb_handle.readline().split(' ')
                        result['V_gene'] = next_line_sp[0]
    ...
    def parse_igblast_tcr(self, chain_type: ChainType, igblast_output: str, **kwargs):
        result_list = []
        result = None
        ...
        with open(igblast_output, 'r') as igb_handle:
            if chain_type in [ChainType.HUMAN_BETA, ChainType.HUMAN_ALPHA, ChainType.MOUSE_BETA, ChainType.MOUSE_ALPHA]:
                while True:
                    line = igb_handle.readline()
                    if not line:
                        break
                    ...
                    if line[:len('Query= ')] == 'Query= ':
                        if result is not None:
                            result_list.append(result)
                        result = {'query': line[len('Query= '):-1], 'hit': False, 'V_gene': '', 'J_gene': '',
                                  'CDR3_NT':'N/A', 'distance_from_V_gene':'N/A'}
                        ...
                        if chain_type == ChainType.HUMAN_ALPHA or chain_type == ChainType.MOUSE_ALPHA:
                            result['V_gene'] = next_line_sp[0]
                            result['J_gene'] = next_line_sp[1]

                        elif chain_type == ChainType.HUMAN_BETA or chain_type == ChainType.MOUSE_BETA:
                            result['V_gene'] = next_line_sp[0]
                            result['D_gene'] = next_line_sp[1]
                            result['J_gene'] = next_line_sp[2]
